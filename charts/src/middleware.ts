...
export function middleware(request: NextRequest) {
    ...
    if (
        ...
        ['/share/prerendered', '/share/fly', '/share/fly/serve'].includes(request.nextUrl.pathname)
    ) {
        const chartParams = [...request.nextUrl.searchParams.entries()].reduce(
            (asObject, [key, val]) => ({ ...asObject, [key]: val }),
            {}
        ) as ChartParams
        chartParams.localeId = 'en-US'
        const encodedChartRouteParam = encodeChartParams(chartParams)
        const encodedUrl = new URL(
            `${request.nextUrl.pathname}/${encodeURIComponent(encodedChartRouteParam)}`,
            request.url
        )
