...
import { fetchLocaleStrings, i18nCommonContexts } from "../db-actions/fetchLocales";

export const cachedFetchLocaleStrings = cache((localeId: string, ...contexts: Array<string>) => fetchLocaleStrings({
    contexts,
    localeId
}))
 ...
 */
export const cachedFetchLocaleFromUrl = cache(async (langParam: string, ...contexts: Array<string>) => {
    ...
    const localeWithStrings = await cachedFetchLocaleStrings(
        localeId,
        ...(contexts.length ? contexts : i18nCommonContexts)
    );
