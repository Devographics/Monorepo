...
const fetchLocaleStringsRedis = async (variables: LocaleStringsVariables) => {
  ...
  if (!keyArray.length) {
    throw new Error("fetchLocaleStringsRedis expects at least one context.")
  }
...
export async function fetchLocaleStrings(variables: LocaleStringsVariables) {
  ...
  const cachePromise = cachedPromise(
    promisesNodeCache,
    cacheKey,
    LOCALES_TTL_SECONDS
  );
  ...
  const locale = await cachePromise(() => fetchLocaleStringsRedis(queryVariables));
