...
test("key and secret key must match", () => {
    ...
    expect(() => checkSecretKey(new NextRequest("http://localhost:3000"))).toThrow()
    expect(() => checkSecretKey(new NextRequest("http://localhost:3000?key="))).toThrow()
    expect(() => checkSecretKey(new NextRequest("http://localhost:3000?key=WRONG-KEY"))).toThrow()
    expect(checkSecretKey(new NextRequest("http://localhost:3000?key=SOME-KEY"))).toBe(true)
    //mock.restore()
...
test("cron_secret must match", () => {
    ...
    expect(() => checkSecretKey(new NextRequest("http://localhost:3000"))).toThrow()
    expect(() => checkSecretKey(new NextRequest("http://localhost:3000", {
        headers: {
            Authorization: ""
        }
    }))).toThrow()
