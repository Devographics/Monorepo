...
export const GET = async (req: NextRequest) => {
    try {
        ...
        if (!editionId) throw new HandlerError({ id: "no-edition-id-params", message: "No editionId in search params" })
        const scoreStr = req.nextUrl.searchParams.get("score")
        if (!scoreStr) throw new HandlerError({ id: "no-score-params", message: "No score in search params" })
        const score = parseInt(scoreStr)
        if (isNaN(score)) throw new HandlerError({ id: "score-nan", message: "Score is not a number" })
        // Store in database
        const SurveyStats = await getSurveyformStatsCollection()
        const globalScore = (await SurveyStats.findOne({ editionId, kind: "quantiles-global-score" }))?.globalScore

        // top 100%
        if (!globalScore) {
            console.warn(`globalScore not found in database for survey ${editionId}, 
            call the '/api/starts/score-quantiles/compute?editionId=${editionId}&key=SECRET_KEY' endpoint to update score quantiles`)
