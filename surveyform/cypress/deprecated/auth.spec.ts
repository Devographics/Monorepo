/**
 * TODO: in state of JS we authenticated on the survey page, not from home
 * We need to adda  few redirections
 */
/**
...
describe("auth", () => {
  ...
  it("login as admin from home and logout", () => {
    ...
    cy.url().should("match", new RegExp(CURRENT_SURVEY_URL, "i"));
    // 3. logout
  ...
  it("signup from home and verify email", () => {
    ...
    cy.findByText(/don\'t have an account|signup/i).click();
    cy.url().should("match", /signup/);
