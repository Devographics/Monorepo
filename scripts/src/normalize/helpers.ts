...
const extractTokens = async ({ value, rules, survey, field, verbose }) => {
    ...
    if (rawString.length > stringLimit) {
        await logToFile('normalization_errors.txt', 'Length Error!  ' + rawString + '\n---\n')
        throw new Error(`Over string limit (${rules.length} rules, ${rawString.length} characters)`)
    }
    ...
    for (const { pattern, context, fieldId, id } of rules) {
        ...
        while (scanCompleted !== true && count < rulesLimit) {
            count++
            if (context && fieldId && (context !== survey.context || fieldId !== field.id)) {
    ...
    tokens.forEach((token, tokenIndex) => {
        ...
        tokens.forEach((nestedToken, nestedTokenIndex) => {
            ...
            if (nestedTokenIndex === tokenIndex || tokensToExclude.includes(nestedTokenIndex))
                return

            // is the nested token contained in the current token range
            if (nestedToken.range[0] >= token.range[0] && nestedToken.range[1] <= token.range[1]) {
                tokensToExclude.push(nestedTokenIndex)
            }
        })
    })

    const filteredTokens = tokens.filter((token, index) => !tokensToExclude.includes(index))
...
export const normalize = async ({
    ...
    const rules = tags ? allRules.filter(r => intersection(tags, r.tags).length > 0) : allRules
...
export const normalizeSource = async (normResp, allRules, survey) => {
    const tags = ['sites', 'podcasts', 'youtube', 'socialmedia', 'newsletters', 'people', 'sources']
    ...
    const rawFindOut = get(normResp, 'user_info.how_did_user_find_out_about_the_survey')
...
export const logAllRules = async () => {
    ...
    if (allEntities) {
        ...
        await logToFile('.logs/rules.js', 'export default ' + json, {
            mode: 'overwrite'
        })
    } else {
