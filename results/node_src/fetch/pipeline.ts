 ...
 */
export async function runFetchPipeline<T>(
    steps: Array<FetchPipelineStep<T>>
    ...
    if (!!steps[steps.length - 1].set) {
        console.warn(
            "Last method of a data fetching pipeline shouldn't have a 'set' method, it is a the source of truth and not a cache"
        )
    }
    if (steps[steps.length - 1].disabled) {
        console.warn(
            'Last method of a data fetching pipeline not be disabled, it is the source of truth'
        )
    }
