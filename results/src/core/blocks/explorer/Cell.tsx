...
const Cell = (props: CellProps) => {
  ...
  const cellData = getCellData({ facet, xTotals, xIndex, respondentsPerDot, percentsPerDot, dotsPerLine, unit });
  ...
  return (
    <Cell_>
      ...
      {showCellCounts && (
        <CellCount_>
          <CellCountInner_>
            {unit === COUNT_UNIT ? cellData.bucketCount : `${Math.floor(cellData.bucketPercentage, 2)}%`}
          </CellCountInner_>
        </CellCount_>
      )}
    </Cell_>
