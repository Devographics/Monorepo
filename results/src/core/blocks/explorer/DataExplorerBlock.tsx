...
const DataExplorerBlock = ({
  ...
  const [percentsPerDot, setPercentsPerDot] = useState(Number(queryParams.percentsPerDot) || PERCENTS_PER_DOT);
  const [dotsPerLine, setDotsPerLine] = useState(Number(queryParams.dotsPerLine) || MAX_DOT_PER_CELL_LINE);
  ...
  */
  ...
  useEffect(() => {
    ...
    const getData = async () => {
      ...
      const query = getQuery(dataExplorerTemplate.query, { surveyType, facet1, facet2, currentYear: surveyYear });
      setQuery(query);
