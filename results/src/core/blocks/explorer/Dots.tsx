...
export const getAllDots = ({
    ...
    const dots = [...Array(TOTAL_DOTS)].map((x, i: number) => {
      ...
      if (peopleCount < totalRespondents) {
        ...
        const facet = facets.find((f: ExplorerDataFacet) => isBetween(peopleCount, f.fromCount, f.toCount));
        ...
        const bucket = facet.buckets.find((b: ExplorerDataBucket) => isBetween(peopleCount, b.fromCount, b.toCount));
        if (!bucket || typeof bucket.columnIndex === 'undefined' || typeof bucket.fromCount === 'undefined') {
