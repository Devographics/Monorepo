...
export const HappinessHistoryBlock = ({ block, data }: HappinessHistoryBlockProps) => {
    ...
    const headings = [
        { id: 'year', label: translate('table.year') },
        { id: 'mean', label: translate('table.mean') }
    ]
    const rows = []
    data.forEach(row => {
        rows.push([
            { id: 'year', label: row.year },
            { id: 'mean', label: `${row.mean}/5` }
        ])
    })
    ...
    return (
        <Block data={data} block={block} tables={tables}>
