...
// TODO: we need to import the list of surveys from a shared folder as well
...
// TODO share types
type NormalizedResponseDocument = any;
...
type PrivateResponseDocument = any;
import { getUUID } from "~/account/email/api/encryptEmail";
import { logToFile } from "~/lib/server/debug";
import {
  getNormalizedResponseCollection,
  getPrivateResponseCollection,
} from "../mongo/models";
...
const logRow = async (columns: Array<any>, fileName: string) => {
  await logToFile(
    `${fileName}.csv`,
  ...
  try {
          ...
          if (privateFieldPaths.includes(normPath)) {
            ...
            if (last(restOfPath) === "others") {
              ...
              if (log) {
                await logToFile(
                  `${fileName}.txt`,
                  `${response._id}, ${fieldName}, ${cleanValue}, ${matchTags.toString()}`,
                );
              }
              try {
                if (verbose) {
                ...
                if (log) {
                  if (normTokens.length > 0) {
                    normTokens.forEach(async (token: any) => {
                ...
                const normIds = normTokens.map((token: any) => token.id);
                const normPatterns = normTokens.map((token: any) =>
    ...
    */
    if (!isEmpty(privateFields)) {
      const privateInfo:
        & Partial<PrivateResponseDocument>
      ...
      const PrivateResponseCollection = await getPrivateResponseCollection();
      ...
      await PrivateResponseCollection.updateOne(
        { responseId: response._id },
        privateInfo,
        { upsert: true },
      );
    ...
    const NormalizedReponseCollection = await getNormalizedResponseCollection();
    const updatedNormalizedResponseRes = await NormalizedReponseCollection
      .findOneAndUpdate(
        { responseId: response._id },
        normResp,
        { upsert: true, returnDocument: "after" },
      );
    const updatedNormalizedResponse = updatedNormalizedResponseRes.value!;
    ...
    return {
      ...
      // TODO: we have to be cautious with string ids...
      // @ts-expect-error
