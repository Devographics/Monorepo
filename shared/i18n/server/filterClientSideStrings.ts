  ...
  async logDebugToCsvFile() {
    ...
    this._unmatchedExprs().forEach((expr) => {
      rows.push([
        expr,
        "NOT FOUND",
        "No tokens from the server dictionnary was matching this token expression. 1) the expression may not be correct 2) the token may not exist 3) you may not load the correct translation contexts.",
      ]);
    });
...
export function filterClientSideStrings<TContext extends Record<string, any>>(
  ...
  if (!getEnvVar(EnvVar.FLAG_ENABLE_STRING_FILTER)) {
    console.warn(
      "Client-side string filtering is not enabled",
      EnvVar.FLAG_ENABLE_STRING_FILTER,
    );
    return locale;
  }
  ...
  // Parse expressions
  const tExprs = rawExprs.map(
    (rawExpr) => new TokenExpr<TContext>(rawExpr, ctx),
  );
  ...
  const clientSideStrings = locale.strings?.filter(
    ({ key }) => key in clientSideDict,
  );
