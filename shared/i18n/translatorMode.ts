...
/**
 * NOTE: can't be disabled, you should refresh the page
 */
export function enableTranslatorMode() {
    ...
    getI18nElements().forEach(el => {
        ...
        translationElem.textContent = `${token || 'EMPTY TOKEN KEY'}|${
            el.textContent || 'EMPTY TRANSLATION'
        }`
        if (missing) {
