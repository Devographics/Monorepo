...
async function getFromRedisOrSource<T = any>({
  ...
  if (redisData) {
    console.debug(
      `🔵 [${key}] in-memory cache miss, redis hit ${calledFromLog}`
    );
    return redisData;
  }
  console.debug(
...
export async function getFromCache<T = any>({
  ...
  try {
    ...
    console.error(err);
    console.debug(
      `🔴 [${key}] error when fetching from Redis or source ${calledFromLog}`
    );
    memoryCache.del(key);
    throw err;
  }
...
export const getApiUrl = () => {
  ...
  if (!apiUrl) {
    throw new Error(
      "process.env.API_URL not defined, it should point the the API"
    );
  }
  return apiUrl;
};
...
export const fetchGraphQLApi = async <T = any>({
  ...
  try {
    ...
    console.error(`// fetchGraphQLApi error 2 (${apiUrl})`);
