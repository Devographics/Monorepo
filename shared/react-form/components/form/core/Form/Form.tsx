...
import { convertSchema, getEditableFields, getInsertableFields } from '../../utils/schema_utils'
...
const getDefaultValues = (convertedSchema: any): any => {
...
const compactObject = (o: any) => omitBy(o, f => f === null || f === undefined)
...
const getChildrenProps = (
    ...
    const formGroupProps = (group: any) => ({
        key: group.name,
        ...group,
...
export const Form = (props: FormProps) => {
    ...
    const { addToFailureForm, addToSubmitForm, addToSuccessForm } = useSubmitCallbacks()
  ...
  */
    const clearFieldErrors = (path: string) => {
    ...
    const addToDeletedValues = (name: string) => {
  ...
  */
    const updateCurrentValues = (newValues: any, options: { mode?: string } = {}) => {
  ...
  */
    ...
    const clearForm = (options: { document?: any } = {}) => {
        ...
        const document = optionsDocument ? merge({}, props.prefilledProps, optionsDocument) : null
        // TODO: prefer a reducer
        setErrors([])
        setCurrentValues({})
        setDeletedValues([])
    ...
    const { successComponent, document, currentUser, model, warnUnsavedChanges } = props
    ...
    return success && successComponent ? (
        ...
        <FormWarnUnsaved isChanged={isChanged} warnUnsavedChanges={warnUnsavedChanges}>
